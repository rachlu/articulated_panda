...
;				)
 ;       :effect (and (not (AtObjConf ?o ?p1)) (not (Holding ?o)) (HandEmpty) (AtObjConf ?o ?p2) (not (AtConf ?q1)) (AtConf ?q2) (CanMove) (Open ?o ?k) (not (AtGraspOpenable ?o ?g ?k)))
  ;  )

    (:action open_obj
        :parameters (?o ?p1 ?p2 ?g ?k ?q1 ?q2 ?i ?s ?t)
        :precondition (and (Openable ?o ?k)
                            (GraspOpenable ?o ?g ?k)
                            (AtGraspOpenable ?o ?g ?k)
                            (not (HandEmpty))
        ...
        :effect (and (not (Holding ?o)) (Open_Amount ?o ?k ?p2))
    )
    ...
    (:derived (Open ?o ?k)
        (exists (?p) (and (Openable ?o ?k) (Open_Amount ?o ?k ?p) (OpenEnough ?o ?k ?p)))
    )
